  ...
  <v-row justify="center">
    <v-dialog v-model="dialogClose" persistent max-width="600px">
      <template v-slot:activator="{ on, attrs }">
        <v-btn color="primary" dark v-bind="attrs" v-on="on"> 영화 등록 </v-btn>
      </template>
      <v-card>
        <v-card-title>
          <span class="text-h5">영화 등록</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12" sm="8">
                <v-text-field
                  label="영화 개봉일"
                  hint="ex) 2021-11-08T00:00:00.000000"
                  persistent-hint
                  required
                  type="text"
                  v-model="openingDate"
...
export default {
  ...
  methods: {
    async saveForm() {
      ...
      try {
        const { data } = await saveMovie(movieData);
    ...
    getMovie(selectedMovie) {
      ...
      // this.openingDate = this.createRandomDate(selectedMovie.pubDate);
